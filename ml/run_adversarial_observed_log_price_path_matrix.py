...
MATRIX_ID = "adversarial-observed-log-price-path-screen-v1"
EPSILON_RMS_VALUES = (
    (0.01, "1e-2"),
    (0.03, "3e-2"),
    (0.1, "1e-1"),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Screen one-step adversarial perturbations of the 121 observed "
            "log-price boundaries while keeping the future endpoint fixed."
        )
    )
...
def main() -> None:
    ...
    for epsilon_rms, epsilon_label in EPSILON_RMS_VALUES:
        suffix = f"adversarial-observed-log-price-eps-{epsilon_label}-v1"
