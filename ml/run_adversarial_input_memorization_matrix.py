...
MATRIX_ID = "adversarial-input-memorization-screen-v1"
EPSILON_RMS_VALUES = (
    (0.01, "1e-2"),
    (0.03, "3e-2"),
    (0.1, "1e-1"),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sweep one-step adversarial input training radii over the six "
            "clean next-second scaling diagnostics."
        )
    )
...
def main() -> None:
    ...
    for base_plan in BASE_PLANS:
        ...
        for epsilon_rms, epsilon_label in EPSILON_RMS_VALUES:
            suffix = f"adversarial-input-eps-{epsilon_label}-v1"
            ...
            commands = (
                ((
                    ...
                    "--adversarial-input-epsilon-rms",
                    str(epsilon_rms),
                    "--adversarial-input-steps",
                    str(ATTACK_STEPS),
                    "--adversarial-input-weight",
