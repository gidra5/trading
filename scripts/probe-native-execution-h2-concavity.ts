/** Falsify or support discrete H2 concavity inside exact guarded H1 request regions. */
...
save("sources.json", Object.fromEntries([
  "scripts/probe-native-execution-h2-concavity.ts", "packages/bot-algo/src/event-execution-one-step.ts",
...
let cases = 0, regions = 0, triples = 0, finiteTriples = 0, violatedTriples = 0, maximumSecondDifference = -Infinity;
for (let sample = 0; sample < samples; sample++) {
  ...
  const child = prepareEventExecutionOneStep(kernel);
  for (const quantity of [-10, -4, -.125, 0, .125, 2, 6, 12]) {
    ...
    const value = (lot: number) => {
      ...
      for (const atom of kernel) {
        ...
        total += atom.probability * (next.logGrowth + child(next).value);
      }
      return total;
    };
    for (const [lo, hi] of cover.requestRegions) {
      ...
      for (let lot = lo + 2; lot <= hi; lot++) {
        ...
        if ([previous, current, next].every(Number.isFinite)) {
          finiteTriples++;
          const second = next - 2 * current + previous;
          maximumSecondDifference = Math.max(maximumSecondDifference, second);
          if (second > 1e-10) {
            violatedTriples++;
...
const summary = { contract: "native-execution-h2-concavity-probe-v1", seed: 519031, samples, cases, regions,
  triples, finiteTriples, violatedTriples, maximumSecondDifference, violations,
  conclusion: violatedTriples ? "Discrete H2 concavity is false inside at least one exact H1 guarded root region."
    : "No counterexample was found in this finite probe; this is not a proof of concavity.",
  scope: "Synthetic exhaustive small-lattice diagnostic with the exact marked H1 child optimizer. It tests a proposed search theorem, not market performance or the production law.",
