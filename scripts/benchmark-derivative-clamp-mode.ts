...
const clampModes: LegacyDerivativeClampMode[] = ["deadband", "hysteresis"];
...
const outputPath = path.resolve(
  repoRoot,
  args.outputPath ?? `data/benchmarks/derivative-clamp-mode-${stamp}.jsonl`,
);
const reportPath = path.resolve(
  repoRoot,
  args.reportPath ?? `docs/derivative-clamp-mode-benchmark-${stamp}.md`,
);
...
function runBenchmark(configArgs: Args, targetOutputPath: string, targetReportPath: string): void {
  ...
  for (const testCase of cases) {
    ...
    for (const clampMode of clampModes) {
      const started = Date.now();
      const result = runBacktestFromCandles(candles, {
        config: benchmarkConfig(configArgs, clampMode),
        maxReturnedOrders: 0,
        maxReturnedFills: 0,
        maxEquityPoints: 32,
        maxChartCandles: 1,
      });
      const row = resultRow(testCase, market, result, configArgs, clampMode, Date.now() - started);
      rows.push(row);
      appendJson(targetOutputPath, row);
      console.error(
        `${testCase.startDate}..${testCase.endDate} ${clampMode}: ${formatSignedPct(row.returnPct)}, DD ${formatPct(row.maxDrawdownPct)}, trades ${row.tradeCount}`,
      );
    }
  }
...
function renderReport(
  ...
  const pairs = cases.map((testCase) => {
    const interval = `${testCase.startDate}..${testCase.endDate}`;
    const deadband = rows.find((row) => row.interval === interval && row.clampMode === "deadband");
    const hysteresis = rows.find((row) => row.interval === interval && row.clampMode === "hysteresis");
    if (!deadband || !hysteresis) {
      throw new Error(`Missing clamp comparison for ${interval}.`);
    }
    return { testCase, deadband, hysteresis };
  });
  const deadbandRows = rows.filter((row) => row.clampMode === "deadband");
  const hysteresisRows = rows.filter((row) => row.clampMode === "hysteresis");
  const improved = pairs.filter((pair) => pair.hysteresis.returnPct > pair.deadband.returnPct).length;
  const lowerDrawdown = pairs.filter(
    (pair) => pair.hysteresis.maxDrawdownPct < pair.deadband.maxDrawdownPct,
  ).length;

  const lines: string[] = [];
  lines.push("# Derivative Clamp Mode Benchmark");
  lines.push("");
  ...
  lines.push(`- Derivative source: \`${configArgs.derivativeSource}\`.`);
  if (configArgs.derivativeSource === "kama") {
  ...
  lines.push("- Compared modes: `deadband` is the previous stateless threshold clamp; `hysteresis` keeps an active derivative sign until the raw derivative crosses zero.");
  lines.push("");
  lines.push("## Summary");
  lines.push("");
  lines.push("| Mode | Avg return | Median return | Positive | Avg DD | Max DD | Avg trades |");
  lines.push("| --- | ---: | ---: | ---: | ---: | ---: | ---: |");
  lines.push(summaryLine("deadband", deadbandRows));
  lines.push(summaryLine("hysteresis", hysteresisRows));
  lines.push("");
  lines.push(`Hysteresis had higher return on \`${improved}/${pairs.length}\` intervals and lower max drawdown on \`${lowerDrawdown}/${pairs.length}\` intervals.`);
  lines.push("");
  lines.push("## Intervals");
  lines.push("");
  lines.push("| Group | Case | Interval | Market | Deadband | Hysteresis | Delta | Deadband DD | Hysteresis DD | Trades |");
  lines.push("| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |");
  for (const pair of pairs) {
    lines.push(
      [
        `| ${pair.testCase.group}`,
        pair.testCase.label,
        `\`${pair.deadband.interval}\``,
        formatSignedPct(pair.deadband.marketReturnPct),
        formatSignedPct(pair.deadband.returnPct),
        formatSignedPct(pair.hysteresis.returnPct),
        formatSignedPct(pair.hysteresis.returnPct - pair.deadband.returnPct),
        formatPct(pair.deadband.maxDrawdownPct),
        formatPct(pair.hysteresis.maxDrawdownPct),
        `${pair.deadband.tradeCount}/${pair.hysteresis.tradeCount} |`,
      ].join(" | "),
    );
  }
  lines.push("");
...
function summaryLine(label: LegacyDerivativeClampMode, rows: ResultRow[]): string {
  return [
    `| \`${label}\``,
