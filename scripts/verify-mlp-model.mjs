...
const PYTORCH_ABSOLUTE_TOLERANCE = 2e-5;
const PYTORCH_RELATIVE_TOLERANCE = 5e-6;
...
if (manifest.inputFeatureCount !== 909 || ![6, 8].includes(manifest.outputParameterCount)
  || manifest.hiddenLayerCount !== 16 || manifest.hiddenWidth !== 1024) {
  throw new Error("MLP manifest does not match the 909 -> 16x1024 -> 6/8 contract.");
}
...
const cpuResult = await cpu.run({ features: input }, ["raw_parameters"]);
const cpuValues = cpuResult.raw_parameters.data;
if (!(cpuValues instanceof Float32Array)
  || cpuValues.length !== batch * manifest.outputParameterCount
...
if (process.env.TRADING_MLP_VERIFY_CUDA !== "false") {
  ...
  const cudaResult = await cuda.run({ features: input }, ["raw_parameters"]);
  const expected = cpuValues;
  const actual = cudaResult.raw_parameters.data;
  if (!(actual instanceof Float32Array)
    || expected.length !== batch * manifest.outputParameterCount
...
function measurePolicyParity(expectedRaw, actualRaw, modelManifest) {
  ...
  const actions = Float64Array.from({ length: 151 }, (_, index) =>
    support.visible_lower
    + index * (support.visible_upper - support.visible_lower) / 150);
  const currents = Float64Array.from({ length: 51 }, (_, index) =>
    support.latent_lower
    + index * (support.latent_upper - support.latent_lower) / 50);
  ...
  for (let batchIndex = 0; batchIndex < batch; batchIndex += 1) {
    const offset = batchIndex * modelManifest.outputParameterCount;
    const options = {
      latentLower: support.latent_lower,
      latentUpper: support.latent_upper,
      visibleLower: support.visible_lower,
      visibleUpper: support.visible_upper,
      friction: support.friction,
      temperature: support.temperature,
    };
    const expectedParameters = conditionalFourSegmentParametersFromRaw(
      expectedRaw.subarray(offset, offset + modelManifest.outputParameterCount),
      options,
    );
    const actualParameters = conditionalFourSegmentParametersFromRaw(
      actualRaw.subarray(offset, offset + modelManifest.outputParameterCount),
      options,
    );
    for (const current of currents) {
      const expected = conditionalFourSegmentExposureProbabilities(
        actions, current, expectedParameters,
      );
      const actual = conditionalFourSegmentExposureProbabilities(
        actions, current, actualParameters,
      );
      let expectedMean = 0;
      let actualMean = 0;
      for (let action = 0; action < actions.length; action += 1) {
        const difference = actual[action] - expected[action];
        ...
        probabilityCells += 1;
        expectedMean += expected[action] * actions[action];
        actualMean += actual[action] * actions[action];
      }
      squaredActionMeanError += (actualMean - expectedMean) ** 2;
      policyRows += 1;
    }
  }
  return {
